...
public class UserDAO {
	...
	/**
	 * 전달받은 Connection을 이용해 DB에 접근하여 전달 받은 아이디와 일치하는 User정보 조회하는 용도
	 * 
	...
	public User selectId(Connection conn, String input) {
		...
		try {
			...
			// 조회 결과가 있을 경우
			// -> 중복되는 아이디가 없을 경우
			// 1행만 조회되기 때문에 while문 보다는 if사용이 효과적
			if (rs.next()) {
				// 첫 행에 데이터가 존재한다면
	...
	public int insertUser(Connection conn, User user) throws Exception {
		...
		try {
			...
			// 5. SQL(INSERT) 수행(executeUpdate()) 후 결과(삽입된 행의 개수) 반환하기
	...
	public List<User> selectAll(Connection conn) throws Exception {
		...
		// 1. 결과 저장용 변수 선언
		// -> List같은 컬렉션을 반환하는 경우에는
		// 변수 선언 시 객체도 같이 생성하는것이 좋다.
		...
		try {
			...
			while (rs.next()) {
				...
				// > TO_CHAR()을 이용해서 문자열로 변환했기 때문!

				// userList에 추가
				// -> User객체를 생성해 조회된 값을 담고
				// userList에 추가한다.
